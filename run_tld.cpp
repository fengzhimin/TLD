	...
	/* Draw debug info: */
		...
		{   /* Draw all information about test sequence: */
			...
			for (i = pTracker->GetBlobNum(); i> 0; i--)
			{
				...
				s.width >>= 8; 
				s.height >>= 8;
				R = Rect(p.x - s.width - 10, p.y - s.height, s.width * 2, s.height * 2 + 2);
...
void detect_and_draw( IplImage* img )
{
	...
	if( cascade )
	{
		...
		for( int i = 0; i < (faces ? faces->total : 0); i++ )
		{
			...
			CvPoint center;
			int radius;
			center.x = cvRound((r->x + r->width*0.5)*scale); //cvRound对一个double型的数进行四舍五入，并返回一个整型数！
			...
			}else
			{
				...
				box.y = center.y-radius*1.35;
				box.width = radius*2;                 //数值为估计的
				box.height = radius*2*1.3;
				times=1;
			}
			cvReleaseImage( &gray );
			cvReleaseImage( &small_img );
